...
maxNumWords = 10
minNumWords = 5  #Double-digit attempts needed to find a random sample that works with these settings
df = df[~df['Talker'].isin(talkerExclude)]
df = df[df['NumWords'].astype(int) <= maxNumWords ]
df = df[df['NumWords'].astype(int) >= minNumWords ]
...
firstNoiseLabels = ['Babble','Clear','Babble','Babble']
firstSoundLabels = ['On','On','On','Off']
primeLabels = ['Off','Off','On','On']
...
dfPickFull['FirstNoise'] = [firstNoiseLabels[x] for x in avOrder]
dfPickFull['FirstSound'] = [firstSoundLabels[x] for x in avOrder]
dfPickFull['Prime'] = [primeLabels[x] for x in avOrder]
dfPickFull['VideoCond'] = [videoLabels[x] for x in avOrder]
...
dfPickFull.to_csv('C:\Experiments\JK308\StudyDesignJK308.csv')