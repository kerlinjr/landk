"""Audio visual presentation of TCD-TIMIT linear noise level
...
#Set paths 
rootPath = normjoin('C:/Experiments/JK307')
stimPath = normjoin('C:/TCDTIMIT/volunteersSmall')
dataOutPath = normjoin(rootPath,'dataOut',subject) 
if practice:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK307Practice_r1.csv'))
elif test:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK307Test_r1.csv'))
else:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK307_r1.csv'))
...
#            wordKeep = range(numWords-3,numWords)
#            wordIdx = numWords-1
...
#        shRange = tt[tt['WordIndex'].isin(wordKeep)]
        ...
        audioOut = (matchSpeech)*db2amp(increaseVolume)    
