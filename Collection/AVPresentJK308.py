"""Audio visual presentation of TCD-TIMIT linear noise level and time added before 3 last words
...
#Set paths 
rootPath = normjoin('C:/Experiments/JK308')
stimPath = normjoin('C:/TCDTIMIT/volunteersSmall')
dataOutPath = normjoin(rootPath,'dataOut',subject) 
if practice:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK308Practice_r1.csv'))
elif test:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK308Test_r1.csv'))
else:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK308_r1.csv'))
...
for trial in np.arange(numTrials):
    ...
    if not test:
        ...
        import random
#        if wordLoc[trial] == 'First3':
#            wordKeep = range(0,3)
        wordNums = range(0,numWords)
        last3 = wordNums[-3:]
        notLast3 = wordNums[:-3]
        #wordIdx = random.sample(range(0,numWords),1)
#        elif wordLoc[trial] == 'Last3':
        #lastWords = range(numWords-3,numWords)
        #gapTime = 1
        #print lastWords
#        wordIdx = numWords-1
#        table['WordIdxList'][trial] = wordIdx

        #wordsToKeep = list(set(range(0,numWords)) - set(wordIdx))
        last3TT = tt[tt['WordIndex'].isin(last3 )]
        notLast3TT = tt[tt['WordIndex'].isin(notLast3)]
        #lastWordsTT = tt[tt['WordIndex'].isin(lastWords)]
        #print lastWordsTT
        #lastWordsOnset = lastWordsTT.reset_index()['OnsetSample'][0]
        shRange = tt
        ...
        #Phonemic Restoration code
        speech1On = table['FirstSound'][trial] == "On" 
        speech2On = 1
        babble1On = table['FirstNoise'][trial] == "Babble" 
        babble2On = 1 
        primeOn = table['Prime'][trial] == "On"
        
        if speech1On:
            matchSpeech[int(notLast3TT['OnsetSample'].iloc[0]):int(notLast3TT['OffsetSample'].iloc[len(notLast3TT)-1])] = speechTmp[int(notLast3TT['OnsetSample'].iloc[0]):int(notLast3TT['OffsetSample'].iloc[len(notLast3TT)-1])] 

        if speech2On:
            matchSpeech[int(last3TT['OnsetSample'].iloc[0]):int(last3TT['OffsetSample'].iloc[len(last3TT)-1])] = speechTmp[int(last3TT['OnsetSample'].iloc[0]):int(last3TT['OffsetSample'].iloc[len(last3TT)-1])]

        if babble1On:
            adjustedBabble[int(notLast3TT['OnsetSample'].iloc[0]):int(notLast3TT['OffsetSample'].iloc[len(notLast3TT)-1])] = babbleTmp[int(notLast3TT['OnsetSample'].iloc[0]):int(notLast3TT['OffsetSample'].iloc[len(notLast3TT)-1])]
        
        if babble2On:    
            adjustedBabble[int(last3TT['OnsetSample'].iloc[0]):int(last3TT['OffsetSample'].iloc[len(last3TT)-1])] =  babbleTmp[int(last3TT['OnsetSample'].iloc[0]):int(last3TT['OffsetSample'].iloc[len(last3TT)-1])]
        ...
        #adjustedBabble[:lastWordsOnset] = 0
        #matchSpeech = np.concatenate([matchSpeech[:lastWordsOnset], np.zeros(int(np.round(48000*gapTime)), dtype=np.int),matchSpeech[lastWordsOnset:]],axis=0)
        audioOut = (matchSpeech + adjustedBabble)*db2amp(increaseVolume)    
        
        for x in np.arange(0,len(tt)):
            wind = np.array([tt['OnsetSample'].iloc[[x]].values[0], tt['OffsetSample'].iloc[[x]].values[0]])
            volume = amp2db(rms(matchSpeech[range(int(wind[0]),int(wind[1]))]))
            print volume
...
#        audioOut = audioOut[int(timeCorrection*48000):]        
    ...
    while core.getTime()-movStart < soundDur+ .1: #mov.status != visual.FINISHED:
        ...
        if 0:#core.getTime()-movStart > lastWordsOnset/float(48000) and core.getTime()-movStart < lastWordsOnset/float(48000) + gapTime: 
            mov.pause()
            trigRestart = 1
            time.sleep(0.005)
        else:
