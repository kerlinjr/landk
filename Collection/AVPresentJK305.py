"""Audio visual presentation of TCD-TIMIT first three and last three words in noise
...
#Set paths 
rootPath = normjoin('C:/Experiments/JK305')
stimPath = normjoin('C:/TCDTIMIT/volunteersSmall')
dataOutPath = normjoin(rootPath,'dataOut',subject) 
if practice:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK305Practice_r1.csv'))
elif test:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK305Test_r1.csv'))
else:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK305_r1.csv'))
