"""Audio visual presentation of TCD-TIMIT linear noise level, yes-no response to target word, before/after
...
#Set paths 
rootPath = normjoin('C:/Experiments/JK311')
stimPath = normjoin('C:/TCDTIMIT/volunteersSmall')
dataOutPath = normjoin(rootPath,'dataOut',subject) 
if practice:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK311Practice_r1.csv'))
elif test:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK311Test_r1.csv'))
else:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK311_r1.csv'))    
    table = table[table['Subject']== int(subject)]
    table = table[table['SubjectTalkerNum']== int(talkerNum)]

corpusList = pd.DataFrame.from_csv("C:\Experiments\JK311\Dictionaries\TCDTIMIT\Dict.txt").transpose()
corpusTrim = []
