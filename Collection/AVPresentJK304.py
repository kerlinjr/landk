"""Audio visual presentation of TCD-TIMIT sentences with missing words
...
#Set paths 
rootPath = normjoin('C:/Experiments/JK304')
stimPath = normjoin('C:/TCDTIMIT/volunteersSmall')
dataOutPath = normjoin(rootPath,'dataOut',subject) 
if practice:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK304Practice_r1.csv'))
elif test:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK304Test_r1.csv'))
else:
    table = pd.read_csv(normjoin(rootPath,'StudyDesignJK304_r1.csv'))
...
#        core.wait(0.1)
     ...
     #   win.flip()
        ...
        myDlg.addField("'Please type the word you didn't hear:'", width = 80,lines =2,multiLineText=True, fontSize=20)
